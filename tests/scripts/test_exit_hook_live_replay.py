...
def test_rows_from_trades_matches_same_ticker_trades_in_order():
    ...
    # pairs/directions. Recovery queues are keyed by (entry_date, ticker,
    # direction) — round 2's fix — so the signals are matched to their OWN
    # trade regardless of the order generate_signals returned them in. This
    # is the re-reviewer's repro: queue order reversed relative to trade_seq
    # order (SHORT signal built/listed before LONG) must still resolve
    # seq 3 (LONG) -> 'A/B' and seq 5 (SHORT) -> 'A/C', with neither trade
    # silently losing its signal to the other's direction.
