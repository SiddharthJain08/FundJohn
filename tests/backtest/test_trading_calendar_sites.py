...
def master(tmp_path, monkeypatch):
    ...
    for d in pd.bdate_range('2019-03-01', '2019-04-26'):
...
def test_backtest_trading_days_skips_good_friday(master, monkeypatch):
...
def test_nearest_monthly_expiry_moves_to_thursday_when_third_friday_is_closed(master, monkeypatch):
    monkeypatch.setattr(tc, '_alpaca_sessions', lambda a, b: (_ for _ in ()).throw(AssertionError('alpaca probe must not run')))
