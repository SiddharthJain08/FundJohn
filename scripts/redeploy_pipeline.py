...
def _run_redeploy(reason: str, run_date: str, dry_run: bool) -> int:
    ...
    if rc != 0:
        return rc
    # OPERATOR RULING 2026-08-10 (supersedes the same-morning signals-only
    # gate that briefly lived here): an intraday regime transition SHOULD
    # submit positions when signals are determined and cross the conviction
    # threshold — full redeploy execution is intended behaviour in BOTH
    # signal-target lanes. The two-wave interplay with the 15:00/15:55
    # same-day chain is accepted; the afternoon lane re-trues (and its
    # zero-conviction flatten overwrites the handoff — sized_handoff
    # pure-flatten exemption, e0342da).
...
def _spawn_orchestrator(reason: str, run_date: str, dry_run: bool,
    ...
    # Signal to the sizer that this is an intraday redeploy. Since the
    # single-λ ruling (2026-08-12) this NO LONGER changes leverage — the
    # retired position_sizing_lambda_intraday key sized the whole 08-12
    # redeploy at a stale 1×. The flag now only drives lane semantics:
    # zero-conviction-flatten exclusion, the thin-signal-set abort, the
    # sized-handoff top-up-lane exclusion, and the corr-diag log-spam guard.
