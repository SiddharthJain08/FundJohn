...
- Live: `engine.load_aux_data` computes the v3 keys on every cycle (flag `OPENCLAW_OPTIONS_SURFACE` unchanged — 0 = shadow, so the keys are summarised, not served, until the flag flips). Shadow line gains `mfiv_nonnull=…% rn_nonnull=…%` (both over the tickers with `n_expiries_fit ≥ 2`, the spec §C.3 denominator) and the §H.5 provenance split `iv30_src smile=…% band=…%` (over the tickers that have an `iv30`).
...
3. Re-backtest owed: §H changes `iv30` (hence `vrp`, `iv_rank`, `ts_ratio`, `term_slope`) for the thin-chain names, and `S21_iv_hv_spread`, `S_HV8_gamma_theta_carry`, `S_HV19_iv_surface_tilt`, `S_HV20_iv_dispersion_reversion` read those keys. After the panel rebuild, re-run those four one strategy at a time (`scripts/rebacktest_options_sleeve.sh` pattern; the 2026-09-06 sleeve unit OOM-killed at 3.4 GB inside S_HV19 under MemoryMax=3500M — use MemoryMax=4500M). The v3-only keys have no consumer; the synthetic engine has no manifest consumer.
