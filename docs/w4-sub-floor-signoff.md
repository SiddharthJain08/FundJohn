...
- **Status:** READ-ONLY. **No live-book change is made from this sheet** — the operator marks KEEP/STOP per row; deprecation happens only on explicit per-row sign-off (same protocol as the W2 manifest↔registry drift sheet).
