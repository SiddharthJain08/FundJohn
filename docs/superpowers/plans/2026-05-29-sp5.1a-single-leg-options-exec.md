...
**Master-data invariant:** Migration 120 is column-ADD only on `alpaca_submissions`. No DROP/DELETE; NULL = legacy 'equity'. Master parquets untouched.
...
|---|---|---|
| `src/database/migrations/120_alpaca_submissions_instrument_class.sql` | ADD COLUMN `instrument_class TEXT NULL` | Create |
| `scripts/apply_migration_120.py` | Idempotent psycopg2 apply | Create |
...
## Task 1: Migration 120 — add `instrument_class` column

**Files:**
- Create: `src/database/migrations/120_alpaca_submissions_instrument_class.sql`
- Create: `scripts/apply_migration_120.py`
...
`scripts/apply_migration_120.py`:
```python
"""Apply migration 120 idempotently via psycopg2. Mirrors mig 119 pattern."""
...
def main():
    sql = open('src/database/migrations/120_alpaca_submissions_instrument_class.sql').read()
    ...
    print("migration 120 applied and verified")
...
```bash
cd /root/openclaw && python3 scripts/apply_migration_120.py
```

Expect: `migration 120 applied and verified`. Re-run → idempotent (no error).

- [ ] **Step 4: Commit**

```bash
git add src/database/migrations/120_alpaca_submissions_instrument_class.sql scripts/apply_migration_120.py
git commit -m "feat(sp5.1a): migration 120 — alpaca_submissions.instrument_class TEXT NULL"
```
...
```bash
git add src/execution/alpaca_executor.py tests/test_options_execution_lane.py
git commit -m "feat(sp5.1a): record_submission writes instrument_class (mig 120 consumer)"
```
...
```
- [ ] Spec doc reviewed
- [ ] Migration 120 applied + verified
