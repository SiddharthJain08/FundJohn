...
**run #4 (post-cache re-time): 10.5 min, run_id `0fa08e0b`, Sharpe +0.2976 — same verdict.** Identical command to run 3 (`--unit=x1-backtest-4`), the only change being the `S_coint_pairs_sector_v2` ledger cache (`6a765a6`): the hook re-read the 860k-row single-row-group pair ledger on every call (~154 ms), once per open leg per bar. 38 min 9 s → 10 min 27 s to `wrote run_id` (the unit still OOM-kills right after it on the known in-process `backtest_panel.rebuild()`; panel rebuilt standalone, `{'built': 1, 'skipped': 0, 'failed': 0}`). Every persisted number is byte-identical to run 3 — Sharpe 0.2976, return +19.95 %, max DD 11.97 %, 1,107 trades, hit 48.4 %, avg hold 6.64 d, exit mix 695 `pair_decohered` / 374 `z_revert` / 24 `max_hold` / 9 `end_of_data` / 5 `stop`, and the same four regime sleeves (TRANSITIONING 0.196 vs bench 1.136, LOW_VOL 1.045 vs 2.196, HIGH_VOL 0.605 vs 1.084 on 59 trades, CRISIS −0.582 on 12) — which is the point: the cache is a pure speed change and X1 remains PARKED on the same benchmark-relative failure. (`config_json.hook_raised` is null on this run; the field landed in `baf1fa8`, after the run started.)

**2026-08-28 — exit-hook Phase 2 (live mirror) landed, flag OFF; X1 still PARKED.** Live `update_pnl` mirror committed (`656c9aa..d23f77e`), parity test 4/4 passed, live replay `AGREEMENT 11/11` — but flip of `OPENCLAW_EXIT_HOOK_LIVE` is BLOCKED pending diagnosis of 3 unexplained early `strategy_exit:*` divergences (see `docs/specs/2026-08-28-per-bar-exit-hook-phase2-spec.md` §4). Activation prerequisite for X1 specifically: align the live hold cap with the qualifying backtest — set `strategy_regime_params.max_hold_days = 30` for its eligible regimes, or re-backtest at the resolver default of 21.
