...
| **C2 — SP-7 §6** | `docs/sp7-phase-c-runbook.md` (live-critical 3-part rollback doc) + `scripts/first_wide_cycle_watcher.py` | Watcher monitors the first **wide** EOD compute (Mon 06-29 16:15 ET) after the clamp deletion; self-removes. Arm its timer (§6) if the 06-29 run is to be auto-watched. |
