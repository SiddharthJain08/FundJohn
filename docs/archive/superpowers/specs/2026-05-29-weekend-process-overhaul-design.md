...
**Semantics:** override is **absolute-replace** — it replaces the strategy's per-signal adaptive stop/target with one flat distance from entry (a fraction) for that (strategy, regime). (Note: this differs from `size_scalar`, which is a *multiplier* composed on top; the override store only holds a flat absolute, so replacement is the available mechanism. The candidate is derived from the strategy's own median stop distance — see A2 — so the flat value stays in the strategy's natural range rather than an arbitrary constant.) Helper `apply_regime_param_override(strategy_id, regime, entry_price, direction, stop, target, *, override_source) -> (stop, target)` is the single shared implementation imported by both call sites (DRY), where `override_source` is either the resolver (live + baseline) or the injected map (candidate).
...
1. **Baseline backtest** — `run_backtest(strategy_id, commit=False, return_metrics=True, param_override=None)` → resolves the strategy's **current persisted** overrides (None on the first run; whatever was applied in a prior week thereafter — gate is ON, so the baseline run *applies* the prior override, meaning its trades reflect current live behaviour). `commit=False` ⇒ the run rolls back (own-conn close) and does **not** persist or rebuild the panel. Returns `total_metrics` carrying `sharpe`, `total_trades`, **and `median_stop_pct` / `median_target_pct`** (computed from the run's trades' recorded `signal_stop`/`signal_target` vs `entry_price`). Record `baseline_sharpe`, `baseline_n_trades`, and these medians.
2. **Derive candidate absolute params.** `stop_delta_pct` / `target_delta_pct` are **dimensionless multiplicative fractions** (confirmed in `position_recommender.js:_deriveDeltas` / `_applyStopReplacements`: e.g. `0.05` = +5%, applied as `× (1 + delta)`). The candidate nudges the strategy's **own current median stop distance** by the delta:
   - `base_stop_pct = baseline.median_stop_pct` (the strategy's real, current effective stop distance — already reflects any prior override), falling back to the engine default **0.07** only when the baseline produced zero trades; `base_target_pct = baseline.median_target_pct` else **0.08**.
   - `candidate_stop_pct = clamp(base_stop_pct * (1 + stop_delta_pct), 0.01, 0.30)`; same for target. Skip the column if its delta is `None` or `|delta| < 0.005` (the recommender's own noise gate).
   - **Sign note:** the override is a flat stop/target *distance* from entry; `+delta` widens the distance. The exact sign-intent is irrelevant to safety — a wrong-direction candidate simply fails the Sharpe gate and is not applied. A test pins the arithmetic.
3. **Candidate backtest** — `run_backtest(strategy_id, commit=False, return_metrics=True, param_override={R: {'stop_pct': candidate_stop_pct, 'target_pct': candidate_target_pct} for R in eligible_regimes})`. Injected map, **no DB write**. Record `candidate_sharpe`, `candidate_n_trades`.
