...
- **TradeJohn confirmer** — RETIRED 2026-07-20. Was a per-ticker news-veto LLM call inside `regime_blended_sizer_live.py` (LOW_VOL/TRANSITIONING, fail-open). Its function — cancel orders on alarming ticker-specific news — is now owned by the pre-market news gate (`src/execution/premarket_gate.py`, `OPENCLAW_EOD_PREMARKET_GATE=1`), which scores FinBERT news + panic on the day's signals (all regimes) before the `trade` step. The `sonnet_premarket_confirmer` provides the optional LLM escalation there. The intraday-redeploy path (`scripts/redeploy_pipeline.py`) also runs the news gate — `_run_intraday_news_gate` splits the redeploy `signals → run_gate() → handoff → trade` (commit 02e7755).
- **PaperHunter** (claude-sonnet-5 since 2026-08-22; Sonnet 4.6 before, upgraded from Haiku 2026-04-23): Per-paper extraction + 4 rejection gates.
- **StrategyCoder** (claude-sonnet-5): On-demand strategy implementation.
- **MastermindJohn** (claude-opus-5, 1M ctx; Opus 4.7 until 2026-08-22): Opus research orchestrator (`src/agent/curators/run_mastermind.js`) + the interactive chat service behind the dashboard Research tab (`mastermind-chat.service`, :7871).
