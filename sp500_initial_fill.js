...
async function main() {
  ...
  try {
    // Load config into collector internals (sets POLYGON_INTERVAL_MS from DB)
    // This is critical: without it, POLYGON_INTERVAL_MS defaults to 13s (free tier)
    const cfg = await collector.loadConfig().catch(async () => {
      return await store.getConfig().catch(() => ({}));
    });
