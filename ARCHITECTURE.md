...
| **Polygon** | 1 | alpha_vantage | 12 | OHLCV, snapshots, market movers, options chain |
...
| **Yahoo** | 2 | — | 6 | Options chains, VIX, insider tx, short interest |
...
The bot itself runs as root, but `claude-bin` subprocesses run as `claudebot` (uid 1001) for sandboxing. MCP tool modules are generated into `workspaces/default/tools/` at startup.
...
│   ├── database/         # migrations + redis client
│   ├── engine/           # cron-schedule.js, strategy-version-manager.js
│   ├── execution/        # engine.py, pipeline_orchestrator.py, post_memos.py, research_report.py, trade_agent.py, runner.js, send_report.py
│   ├── ingestion/        # hardcoded 3-layer async ETL (replaces DataJohn) — pipeline.py + edgar_client.py
...
- **Dashboard** — web UI (if enabled) — reads Redis heartbeat keys written by `pipeline-activity.js`.
