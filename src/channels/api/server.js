...
function _renderActiveStack(rows) {
  ...
  el.innerHTML = \`<table class="db-table st-active-table" style="min-width:1180px">
    <tr>
      ...
      <th class="num" data-sort-key="_forward_er" data-sort-type="num" title="Forward ER projection = ARR × per-regime OUE multiplier. The multiplier uses TIME-DECAYED OUE counts (τ=45d half-life ≈ 31d) so it responds to recent behavior — a strategy improving this month will see its discount lift within ~6 weeks. Multiplier in [0.20, 2.00]; defaults to 1.00 until weighted sample reaches 30 closes with ≥5 outliers in the regime.">Fwd&nbsp;ER&nbsp;%\${filterTag}</th>
